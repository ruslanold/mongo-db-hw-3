...
db.getCollection('students').find({"lessons": { $eq :"physics"}})

// 4) Знайти всіх дітей, батьки яких працюють в науці ( scientist )
db.getCollection('students').find({"parents.profession": "scientist" })

// 5) Знайти дітей, в яких середня оцінка більша за 4
db.getCollection('students').find({avgScore: { $gt: 4.2}})
...
db.getCollection('students').find({"lessons.2": {$exists: 1}}).sort({ avgScore : -1}).limit(3)

// 9) Знайти середній бал по школі
db.getCollection('students').aggregate([{
    $group: {_id: 0, avg:{ $avg: "$avgScore"}}
}])
...
db.getCollection('students').find({parents:{$elemMatch:{"profession":{$exists: 0}}}})

// 14) Не працюючих батьків влаштувати офіціантами
db.getCollection('students').update({parents:{$elemMatch:{"profession":{$exists: 0}}}},
...
db.getCollection('students').update({parents:{$elemMatch:{"profession":{$eq: "teacher"}}}},
                                    {$set: {avgScore: 5}},
                                    {multi: 1})

// 17) Знайти дітей які вчаться в початковій школі (до 5 класу) і вивчають фізику ( physics )
db.getCollection('students').find({$and:[{class: {$lte: 5}}, {lessons: {$eq: "physics"}}]})
